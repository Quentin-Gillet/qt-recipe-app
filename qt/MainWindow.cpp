...
MainWindow::MainWindow(QWidget *parent)
  ...
  label->setGeometry(this->width() / 2 - labelWidth / 2, -90, labelWidth,
                     labelWidth);
  label->show();
  ...
  // Refresh button
  connect(this->ui->refreshButton, &QPushButton::clicked, recipeGrid,
          &RecipeGrid::refreshRecipeGrid);

  // Menu bar
  // Search
  connect(this->searchAction, &QAction::triggered, this,
          &MainWindow::displaySearchDialog);
  // Discover
  connect(this->exploreAction, &QAction::triggered, recipeGrid,
          &RecipeGrid::refreshRecipeGrid);
  // Favourite
  connect(this->favouriteAction, &QAction::triggered, this,
          &MainWindow::displayFavourites);
}

MainWindow::~MainWindow() { delete ui; }

void MainWindow::performSearch(const QString &searchQuery,
                               const QString &mealDiet, int maxCalories,
                               bool ignorePantry, int resultsCount) {
  Scrapper scrapper;
  bool success;
  QList<Recipe *> recipes = scrapper.searchRecipe(
      &success, searchQuery, mealDiet, maxCalories, ignorePantry, resultsCount);
  if (!success) {
    recipeGrid->clearRecipeGrid();
    recipeGrid->showMessage("Error while loading recipes from API. Try again later");
    return;
  }
  recipeGrid->clearRecipeGrid();
